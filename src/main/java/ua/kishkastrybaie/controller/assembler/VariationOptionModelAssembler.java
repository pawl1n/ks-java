...
public class VariationOptionModelAssembler implements RepresentationModelAssembler<VariationOption, VariationOptionDto> {
    ...
    public VariationOptionDto toModel(@NonNull VariationOption variationOption) {
        return variationOptionMapper.toDto(variationOption)
                .add(linkTo(methodOn(VariationOptionController.class).one(variationOption.getId())).withSelfRel());
    }

    @Override
    @NonNull
    public CollectionModel<VariationOptionDto> toCollectionModel(@NonNull Iterable<? extends VariationOption> variationOptions) {
        return RepresentationModelAssembler.super.toCollectionModel(variationOptions)
